...
- ✅ **CORRECTED 2026-08-05: the site DOES auto-deploy from `git push` to `master`.** Verified live — pushing `a084482` produced a CF Pages **Production** deployment on branch `master` within ~1 min, and the apex served it. The note below is retained for history but is no longer accurate. ⚠️ If deploying manually, the production branch is **`master`**, not `main` — `wrangler pages deploy dist --branch=main` creates a *Preview* deploy that never reaches the apex. Simplest path: just `git push`.
- 🕘 *(superseded)* ~~Site does NOT auto-deploy from `git push`. Deploy method is Wrangler CLI.~~ Build then run wrangler from the repo's `dist/`. CF API token is in the central master-builds store (Drive-only, gitignored) — never commit it. (Worth flipping CF Pages → GitHub auto-deploy in dashboard; Sunny TODO.)
...
- **Aug 4** — `/about/` gains a "Delivery Partners" section naming **Callum Dice / TotalOps Automation** as *automation delivery partner* (commit `1b69596`, local only, NOT pushed/deployed pending Sunny's wording sign-off). Deliberately NOT "Head of AI Ops" — a staff claim AAA can't evidence is an E-E-A-T liability (same shape as the redlighttherapy.expert fake-expert demotion). Dubai-division and "3 years ML" claims from his supplied bio were dropped as unverifiable. 🔴 **Links the TOA homepage only, never `totalopsautomation.com/ai-llm-seo`** — linking AAA to a partner's SEO page breaches the Figment positioning lock above. Related: the TOA partnership sends *SEO/AI-visibility* leads to Sunny, which AAA cannot receive; those must route to SunnyPatel.co.uk.
